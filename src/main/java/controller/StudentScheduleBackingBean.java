...
@Named
@ViewScoped
public class StudentScheduleBackingBean implements Serializable {
    ...
    public void init() {
        ...
        this.dateFormatter = new SimpleDateFormat("E MMM dd HH:mm:ss z yyyy");
    ...
    public void scheduleSetup(){
        for(CourseModule cm : moduleList){
            eventModel.addEvent(new DefaultScheduleEvent(cm.getTopic(),stringToDate(cm), stringToDate(cm)));
        }
    }
    
    public Date stringToDate(CourseModule cm){
        String cmDate = cm.getWeek();
        Date date = null;
        try {
            date = dateFormatter.parse(cmDate);
        } catch (ParseException ex) {
            Logger.getLogger(StudentScheduleBackingBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return date;
    }
}
